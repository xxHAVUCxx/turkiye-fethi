document.addEventListener('DOMContentLoaded', () => {
    ...
    function startPlayerTurn() {
        ...
        minigame.timeLeft = 10.0;
        minigame.timerEl.textContent = "10.0s";
    ...
    function startMathGame(onSuccess, onFail) {
        ...
        mathGame.timerEl.textContent = "10.0s";
    ...
    */
    ...
    if (socket) {
        ...
        socket.on('cell_conquered', (data) => {
            ...
            // Yeni bölge fethedilince o oyuncunun KALESİNDEKİ metrekare yazısını dinamik güncelle
            if (data.owner) {
                updateCastleTooltip(data.owner);
            }
        });

        // Kalenin üzerindeki bilgi penceresinin (tooltip) dinamik olarak güncellenmesi
        function updateCastleTooltip(ownerEmail) {
            if (!window.serverMapState || !window.gridCells) return;
            
            let castleId = null;
            let ownerArea = 0;
            let color = '#fff';
            let emoji = '🏰';
            let ownerName = 'Oyuncu';

            for (const cid in window.serverMapState) {
                const state = window.serverMapState[cid];
                if (state.owner === ownerEmail) {
                    ownerArea += 5;
                    if (state.isCastle) {
                        castleId = cid;
                        color = state.color;
            ...
            if (castleId && window.gridCells[castleId] && window.gridCells[castleId].marker) {
                const areaText = ownerArea >= 1000000 ? 
                    (ownerArea / 1000000).toLocaleString('tr-TR', { maximumFractionDigits: 1 }) + " km²" : 
                    ownerArea.toLocaleString('tr-TR') + " m²";
                ...
                const marker = window.gridCells[castleId].marker;
        ...
        socket.on('force_disconnect', (data) => {
            alert(data.reason || "Bağlantınız kesildi.");
            window.location.reload(); // Sayfayı yenileyerek tamamen çıkış yapmasını sağla
        });
